...
class gammu {
	...
	function __construct($gammu_bin_location='',$gammu_config_file='',$gammu_config_section='')
	{
		...
		if (!file_exists($this->gammu)) {
			$this->error("Gammu not found <b><u>{$this->gammu}</u></b> or not installed\r\n");
		} else {
	...
	function Get()
	{
		...
		for($i=0;$i < count($r);$i++)
		{
			...
			//unset$data[$folder][$fid]['status']='';//we dont want it
